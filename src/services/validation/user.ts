...
const user: Type = {
  ...
  location: {
    firstName: [isRequired, minMaxLength({ min: 2, max: 25 })],
    lastName: [isRequired, minMaxLength({ min: 2, max: 25 })],
    address_1: [isRequired, minMaxLength({ min: 5, max: 50 })],
    city: [isRequired, minMaxLength({ min: 5, max: 50 })],
